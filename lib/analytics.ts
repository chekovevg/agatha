...
export function canLoadAnalytics(hostname: string, gtmId: string | undefined, consent: AnalyticsConsent | null): boolean {
...
export function writeAnalyticsConsent(storage: Pick<Storage, "setItem">, consent: AnalyticsConsent): void {
  storage.setItem(ANALYTICS_CONSENT_KEY, consent);
}

export function pushAnalyticsEvent(event: AnalyticsEvent): boolean {
  try {
    if (typeof window === "undefined" || !productionHosts.has(window.location.hostname) || readAnalyticsConsent(window.localStorage) !== "granted") {
      return false;
    }
...
export function calPathFromUrl(url: string): string | null {
  try {
    ...
    if (parsed.protocol !== "https:" || parsed.hostname !== "cal.com" || parsed.username || parsed.password) return null;
...
export function createBookingSuccessTracker(push: (event: AnalyticsEvent) => unknown): (notification: unknown) => void {
  let tracked = false;
  return (notification) => {
    if (tracked) return;
    const title = (notification as {detail?: {data?: {title?: unknown}}} | null)?.detail?.data?.title;
    const bookingType = title === "Intro Call" ? "intro_call" : title === "Music Lesson" ? "music_lesson" : null;
