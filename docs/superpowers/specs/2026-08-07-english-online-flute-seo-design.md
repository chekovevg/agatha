...
hidden referrer persistence, cross-page UTM storage, or a new analytics system.
...
Use `Flute & Music Teacher` as the visually dominant first line of the home H1
and `For Adults and Children` as its second line. Keep the complete existing
hero visual, including the musical-staff treatment, unchanged in this
iteration. Preserve `online flute lessons` in metadata, supporting copy, and
the audience landing pages so the more personal home positioning does not
replace the transactional search language.

Shorten the home page to this order:

1. existing hero with a primary `Get in Touch` action linking to `/book`;
...
hero visual remains unchanged, the manifesto buttons are visible and usable,
...
Web Vitals in Speed Insights before changing fonts, preloads, or the WebGL hero.
Do not expand into German localization, GTM, a blog, local doorway pages, or
additional lesson-service pages until observed demand justifies the work.
