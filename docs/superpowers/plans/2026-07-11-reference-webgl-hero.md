...
- `PASS_DESCRIPTORS` is the single ordered render graph for both program
  compilation and frame execution. It fixes background, vignette, sine,
  shatter, bokeh, and composite order together with each pass's output target,
  texture bindings, units, and runtime uniforms; do not duplicate this order in
  a second hard-coded call sequence or derive it from object key iteration.
